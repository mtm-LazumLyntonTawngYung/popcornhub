...
class PopcornHubChatbot:
    def __init__(self):
        # Load model
        self.model = joblib.load(os.path.join('models', 'intent_clf.pkl'))

        # Load intents
        with open(os.path.join('data', 'intents.json'), 'r') as f:
            self.intents = json.load(f)
        self.intent_responses = {intent['tag']: intent['responses'] for intent in self.intents}

        # Load knowledge base
        with open(os.path.join('data', 'kb_sentences.txt'), 'r') as f:
        ...
        # Memory
        self.db = TinyDB('chatbot_memory.json')
